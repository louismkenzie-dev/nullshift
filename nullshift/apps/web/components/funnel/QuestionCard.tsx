 ...
 *  reveals a free-text input instead of advancing. A step with `kind: "text"`
 *  renders a free-text box (e.g. "Describe your business"). Accessible: the
 *  question is a focus-managed heading, options are a labelled button group. */
export function QuestionCard({
  ...
  const [text, setText] = useState(step.kind === "text" ? (selected ?? "") : "");
  ...
  return (
    <div>
      ...
      {/* Free-text question (e.g. describe your business) */}
      {isText && (
        <div className="mt-8">
          <textarea
            ref={textRef}
          ...
          <button
            ...
            onClick={submitText}
            disabled={!text.trim()}
            className="mt-3 inline-flex items-center justify-center font-medium disabled:opacity-50"
            style={{
              height: 46,
              paddingInline: 22,
              background: T.primary,
            ...
            return (
              <button
                ...
                style={{
                  ...
                  transition:
                    "border-color .18s ease, background .18s ease, transform .12s ease",
                }}
              >
