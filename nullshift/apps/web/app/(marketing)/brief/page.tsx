...
function BriefInner() {
        ...
        >
          <div className="max-w-lg w-full">
            ...
            <p style={{ fontFamily: T.sans, color: T.muted, marginBottom: "32px" }}>
              Thanks, <span style={{ color: T.fg }}>{form.clientName}</span>. We&apos;ll
              review your brief and be in touch within 1–2 business days.
            </p>
            <div
            ...
            >
              ...
              <Row
                label="Pages"
                value={form.pages.map((p) => labelFor(PAGE_OPTIONS, p)).join(", ")}
              />
              ...
              <Row
                label="Budget"
                value={form.budget >= BUDGET_MIN ? formatBudget(form.budget) : "—"}
              />
  ...
  return (
    <>
      ...
      >
        <header
          className="px-6 pt-24 pb-6 flex items-center justify-between"
        ...
        <div className="flex-1 flex flex-col items-center justify-start py-12 px-4">
          <div className="w-full max-w-xl">
            ...
            <div className="flex items-center gap-2 mb-10">
              {STEPS.map((s, i) => (
                <div key={s.id} className="flex items-center gap-2">
                  ...
                  {i < STEPS.length - 1 && (
                    <div
                      className="h-px w-6 sm:w-8 ml-1"
                      style={{ background: s.id < step ? `${T.primary}55` : T.border }}
                    />
                  )}
                </div>
              ))}
            ...
            >
              ...
              {step === 2 && (
                <StepTwo form={form} errors={errors} togglePage={togglePage} set={set} />
              )}
            ...
            {submitError && (
              <p
                ...
                style={{ fontFamily: T.mono, fontSize: 11, color: T.danger }}
              >
            ...
            <div className="flex items-center justify-between mt-6">
              ...
              <button
                onClick={next}
                disabled={submitting}
                className="flex items-center gap-2 px-6 py-2.5 rounded-md transition-opacity hover:opacity-90 disabled:opacity-50"
                style={{
                  fontFamily: T.mono,
                  fontSize: "12px",
                  fontWeight: 500,
...
function StepOne({
  ...
  form: BriefData;
  errors: Errs;
  set: <K extends keyof BriefData>(k: K, v: BriefData[K]) => void;
}) {
  return (
    <div className="space-y-5">
      <StepHeading
        marker="// 01 — Contact"
        title={"Let's start\nwith you."}
        sub="Basic contact details so we can follow up."
      />
  ...
  return (
    <div className="space-y-5">
      <StepHeading
        ...
        sub="Select all that apply. You can add custom pages below."
      />
          ...
          return (
            ...
            >
              <div
                ...
                style={{
                  border: `1px solid ${checked ? T.primary : `${T.muted}55`}`,
                  background: checked ? T.primary : "transparent",
                }}
              >
                {checked && (
                  <Check className="w-2.5 h-2.5" style={{ color: T.primaryFg }} />
                )}
              </div>
              {page.label}
            </button>
      ...
      <Field label="Any other pages?" note="optional">
        <input
          ...
          placeholder="e.g. Case studies, Press, Login"
          className={inputCls(false)}
        />
      </Field>
    </div>
  ...
  return (
    <div className="space-y-6">
      <StepHeading
        ...
        sub="What direction do you want the design to take?"
      />
        ...
        <div className="grid gap-2">
          {DESIGN_STYLES.map((style) => {
            ...
            return (
              <button
                key={style.id}
                onClick={() => set("designStyle", style.id)}
                className="flex items-center justify-between px-4 py-3 rounded-md text-left transition-all"
                style={{
                  border: `1px solid ${sel ? `${T.primary}80` : T.border}`,
                  background: sel ? `${T.primary}0d` : `${T.surface2}66`,
                }}
              >
                <div>
                  <span className="text-sm block" style={{ color: sel ? T.fg : T.muted }}>
                    {style.label}
                  </span>
                ...
                <div
                  className="w-4 h-4 rounded-full shrink-0 ml-4 flex items-center justify-center"
                  style={{ border: `1px solid ${sel ? T.primary : T.border}` }}
                >
                  {sel && (
                    <div
      ...
      {(form.hasLogo === "yes" || form.hasLogo === "wip") && (
        <Field label="Logo / branding notes" note="optional">
          <textarea
            ...
            rows={3}
            placeholder="Existing brand colours, fonts, files…"
  ...
  return (
    <div className="space-y-6">
      <StepHeading
        ...
        sub="What do you want the website to achieve?"
      />
      <div className="grid gap-2">
        {PURPOSES.map((p) => {
          ...
          return (
            <button
              key={p.id}
              onClick={() => set("websitePurpose", p.id)}
  ...
  return (
    <div className="space-y-6">
      <StepHeading
        ...
        sub="Helps us scope the right solution for you."
      />
      <div className="space-y-4">
        <div className="flex items-end justify-between">
          ...
          <span
            style={{
              ...
              fontSize: "22px",
              lineHeight: 1,
              color: form.budget >= BUDGET_MIN ? T.primary : T.muted,
            }}
          >
        ...
        <div className="relative py-3">
          ...
          />
          <div
            className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 rounded-full pointer-events-none transition-all"
            style={{
              left: `${pct}%`,
              background: T.primary,
          ...
          {TIMELINES.map((t) => {
            ...
            return (
              <button
                ...
                className="py-2.5 px-4 rounded-md text-sm text-left transition-all"
                style={{
                  border: `1px solid ${sel ? `${T.primary}80` : T.border}`,
                  background: sel ? `${T.primary}0d` : `${T.surface2}66`,
                  color: sel ? T.fg : T.muted,
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <label
      ...
      style={{ fontFamily: T.sans, fontWeight: 500, color: T.fg }}
    >
      {children}
    </label>
  );
...
function FieldError({ msg }: { msg?: string }) {
  ...
  return (
    <p
      ...
      style={{ fontFamily: T.mono, fontSize: "11px", color: T.danger }}
    >
...
function inputCls(err: boolean) {
  return `brief-input${err ? " brief-input-err" : ""}`;
}
