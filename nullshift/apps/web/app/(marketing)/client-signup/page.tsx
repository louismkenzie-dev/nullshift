...
export default function ClientSignupPage() {
  ...
  useEffect(() => {
    try {
      if (email.trim()) localStorage.setItem("ns_email", email.trim());
    } catch {
      /* ignore */
    }
  }, [email]);
  ...
  function validateStep(): boolean {
    ...
    if (cur === "email" && (!email.trim() || !EMAIL_RE.test(email.trim()))) {
          ...
          {phase === "form" ? (
            <>
              ...
              {error && (
                <p
                  className="mt-4"
                  style={{ fontFamily: T.mono, fontSize: "12px", color: T.danger }}
                >
                  {error}
                </p>
              )}
            ...
            <div>
              ...
              >
                ...
                <span style={{ color: T.fg }}>{email}</span>. Enter it to confirm your
                account.
              </p>
                ...
                {resendMsg && (
                  <p
                    ...
                    style={{
                      fontFamily: T.sans,
                      fontSize: "0.8125rem",
                      color: resendMsg.includes("Could not") ? T.danger : T.primary,
                    }}
                  >
                    {resendMsg}
                  </p>
...
function StepBody(props: {
  ...
  prefDate: string;
  setPrefDate: (v: string) => void;
  prefTime: string;
  setPrefTime: (v: string) => void;
  password: string;
  ...
  const Sub = ({ children }: { children: React.ReactNode }) => (
    <p
      ...
      style={{ fontFamily: T.sans, fontSize: "0.95rem", lineHeight: 1.6, color: T.muted }}
    >
      {children}
    </p>
  );
      ...
      <div>
        ...
        <div className="mt-7 flex flex-col gap-3">
          <input
            ...
            value={props.prefDate}
            onChange={(e) => props.setPrefDate(e.target.value)}
          />
            ...
            {[
              ...
              return (
                <button
                  key={id}
                  type="button"
                  onClick={() => props.setPrefTime(id)}
                  className="ns-funnel-option"
                  aria-pressed={sel}
                  style={{
                    height: 48,
                    borderRadius: T.r.md,
  ...
  return (
    <div>
      ...
      <H>Set a password.</H>
      <Sub>
        This creates your client account
        {props.email ? (
          <>
            {" "}
            for <span style={{ color: T.fg }}>{props.email}</span>
          </>
        ) : null}
        .
      </Sub>
      <div className="mt-7 flex flex-col gap-3">
        ...
        <input
          ...
          onChange={(e) => props.setConfirmPw(e.target.value)}
          onKeyDown={enter}
        />
      </div>
    </div>
