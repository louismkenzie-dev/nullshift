  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6" style={{ background: T.bg }}>
      <div className="w-full max-w-sm">
        ...
        {step === "form" ? (
          <form onSubmit={handleSignup} className="flex flex-col gap-4 p-8 rounded-2xl" style={{ background: T.surface, border: `1px solid ${T.border}`, boxShadow: T.shadow.md }}>
            <div className="flex flex-col gap-1.5">
              <label style={{ fontFamily: T.sans, fontSize: "0.75rem", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", color: T.muted }}>Full name</label>
              <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="Your name" style={inputBase} autoComplete="name" onFocus={onFocus} onBlur={onBlur} />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" style={inputBase} autoComplete="email" onFocus={onFocus} onBlur={onBlur} />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 8 characters" style={inputBase} autoComplete="new-password" onFocus={onFocus} onBlur={onBlur} />
            </div>

            <div className="flex flex-col gap-1.5">
              <label style={{ fontFamily: T.sans, fontSize: "0.75rem", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", color: T.muted }}>Confirm password</label>
              <input type="password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Repeat password" style={inputBase} autoComplete="new-password" onFocus={onFocus} onBlur={onBlur} />
            </div>
          ...
          <form onSubmit={handleVerify} className="flex flex-col gap-5 p-8 rounded-2xl" style={{ background: T.surface, border: `1px solid ${T.border}`, boxShadow: T.shadow.md }}>
            <p style={{ fontFamily: T.sans, fontSize: "0.9375rem", lineHeight: 1.65, letterSpacing: "-0.005em", color: T.muted }}>
              We sent a <strong style={{ color: T.fg, fontWeight: 600 }}>6-digit code</strong> to <strong style={{ color: T.fg, fontWeight: 600 }}>{email}</strong>. Enter it below to verify your account.
            </p>
              ...
              {digits.map((d, i) => (
                <input
                  ...
                  style={{
                    ...
                    borderRadius: T.r.md,
                    color: T.fg,
                    fontFamily: T.mono, fontWeight: 600, fontSize: "1.4rem",
            ...
            <button
              ...
              style={{ width: "100%", fontFamily: T.sans, fontSize: "0.9375rem", fontWeight: 500, letterSpacing: "-0.005em", background: T.primary, color: T.primaryFg, borderRadius: T.r.md, border: "none", boxShadow: `inset 0 1px 0 rgba(255,255,255,0.18)`, transition: `background ${T.duration.base} ${T.ease}`, opacity: (busy || digits.join("").length < 6) ? 0.4 : 1 }}
            >
              {busy ? "Verifying…" : "Verify & sign in →"}
            </button>

            <p style={{ fontFamily: T.sans, fontSize: "0.8125rem", letterSpacing: "-0.003em", color: T.muted, textAlign: "center" }}>
              Didn&apos;t receive it?{" "}
              <button type="button" onClick={handleResend} disabled={resendBusy}
                style={{ color: T.primary, background: "none", border: "none", cursor: "pointer", fontFamily: T.sans, fontSize: "0.8125rem", opacity: resendBusy ? 0.6 : 1, textDecoration: "underline" }}>
        ...
        <div className="mt-6 text-center flex flex-col gap-3">
          <Link href="/portal/login" style={{ fontFamily: T.sans, fontSize: "0.8125rem", color: T.muted, textDecoration: "none" }}>
            Already have an account? Sign in →
          </Link>
          <Link href="/" style={{ fontFamily: T.sans, fontSize: "0.8125rem", color: T.faint, textDecoration: "none" }}>
            ← Back to website
          </Link>
        </div>
      </div>
