  ...
  try {
    ...
    if (!clientId || !email || !password) {
      return NextResponse.json(
        { error: "clientId, email and password are required." },
        { status: 400 }
      );
    }
    ...
    if (!userId) {
      return NextResponse.json(
        { error: "Account creation failed — no user ID returned." },
        { status: 500 }
      );
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        {
          error:
            "Account created but could not be linked to the client record. Contact support.",
        },
        { status: 500 }
      );
    }
