...
import { loadAgreement, todayIso } from "@/lib/ops/agreementsData";
...
export default async function AgreementWorkspace({
  ...
  const record = await loadAgreement(id);
  if (!record) notFound();
  const d = record.doc;

  // Review state from the raw uuids (author ≠ reviewer), displayed with names.
  const review = { ...reviewState(record.review), ...d.review };
  const blockers = issueBlockers(d);
  const preIssue = PRE_ISSUE_STATUSES.includes(d.status);
  const issue = canIssue(d, review);
  const exp = expiryState(d, todayIso());

  return (
    <>
      <p className={s.mono}>
        <Link href="/admin/agreements">Agreements</Link> /{" "}
        <Link href={`/admin/clients/${d.clientId}`}>{d.client}</Link> / {d.type}{" "}
        {d.version}
      </p>
      <div className={s.pageHead}>
          ...
          <p className={s.lead}>
            ...
            {d.supersedes ? (
              <>
                ...
                · supersedes{" "}
                <Link
                  href={`/admin/agreements/${d.supersedes}`}
                  className={s.rowLink}
                >
                  {d.supersedes}
                </Link>
            ...
            {d.supersededBy ? (
              <>
                ...
                · superseded by{" "}
                <Link
                  href={`/admin/agreements/${d.supersededBy}`}
                  className={s.rowLink}
                >
                  {d.supersededBy}
                </Link>
        ...
        <div className={s.chips}>
          <span
            className={`${s.chip} ${issue.ok ? s.chipSuccess : s.chipWarning}`}
            title={issue.ok ? "All gates pass" : issue.reasons.join(" · ")}
          >
            {issue.ok ? "Issue gates pass" : `${issue.reasons.length} issue gate${issue.reasons.length === 1 ? "" : "s"} open`}
          </span>
          <Link href={record.editorHref} className={s.btnPrimary}>
            Open in editor
          </Link>
        </div>
      </div>
      ...
      {d.legacy ? (
        <div className={a.legacyNote} role="note">
          <strong>Legacy proposal — read-only</strong>
          <span>
            Recorded under the proposal regime on projects.proposal_status. It is never
            regenerated from today&apos;s catalogue and is never edited in place; the
            client hub shows the original.
          </span>
        </div>
      ) : null}
        ...
        <div className={s.stack}>
          <section className={s.card} aria-labelledby="facts">
            ...
            <dl className={s.kv}>
              ...
              <dd>
                {d.facts.governing ? (
                  <Link
                    href={`/admin/agreements/${d.facts.governing}`}
                    className={s.rowLink}
                  >
                    {d.facts.governing}
                  </Link>
          ...
          <section className={s.card} aria-labelledby="pricing">
            <div className={s.cardTitle}>
              ...
              <span className={s.mono}>as recorded on the row</span>
            </div>
            {d.pricing.length === 0 ? (
          ...
          <section className={s.card} aria-labelledby="evidence">
            <div className={s.cardTitle}>
              <h2 className={s.h2} id="evidence" style={{ margin: 0 }}>
                Acceptance evidence
              </h2>
              <span className={s.mono}>contract_acceptances</span>
            </div>
            {record.evidence ? (
              <dl className={s.kv}>
                <dt>Accepted by</dt>
                <dd>
                  {record.evidence.acceptedByName}, {record.evidence.acceptedByTitle} ·{" "}
                  {record.evidence.acceptedByEmail}
                </dd>
                <dt>Accepted at</dt>
                <dd>{record.evidence.acceptedAt.replace("T", " ").slice(0, 16)} UTC</dd>
                <dt>Method</dt>
                <dd>{record.evidence.method}</dd>
                <dt>IP · user agent</dt>
                <dd>
                  {record.evidence.ipAddress ?? "—"}
                  <div className={a.auditNote}>{record.evidence.userAgent ?? "—"}</div>
                </dd>
                <dt>Versions accepted</dt>
                <dd>
                  {record.evidence.versions
                    .filter((v) => v.value)
                    .map((v) => `${v.label} ${v.value}`)
                    .join(" · ")}
                </dd>
                <dt>Documents hashed</dt>
                <dd>{record.evidence.hashedDocuments.join(", ") || "none"}</dd>
                <dt>Signed artifact</dt>
                <dd>{record.evidence.signedArtifactPath ?? "not stored"}</dd>
              </dl>
            ) : (
              <p className={s.muted} style={{ margin: 0 }}>
                {d.status === "Accepted"
                  ? "Accepted without a contract_acceptances row (recorded on the document itself)."
                  : "No acceptance recorded. Sent and viewed events are evidence only."}
              </p>
            )}
          </section>
          ...
          <section className={s.card} aria-labelledby="links">
            ...
            <ul className={s.list}>
              <li className={s.listItem}>
                ...
                <Link href={`/admin/clients/${d.clientId}`} className={s.rowLink}>
                  {d.client}
                </Link>
              </li>
              {d.facts.quoteRef ? (
              ...
              <li className={s.listItem}>
                ...
                <Link
                  href={`/admin/agreements?client=${d.clientId}`}
                  className={s.rowLink}
                >
                  Library
                </Link>
