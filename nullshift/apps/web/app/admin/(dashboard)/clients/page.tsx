...
export default function ClientsPage() {
  ...
  const load = useCallback(async () => {
    ...
    for (const p of (props ?? []) as {
      ...
      if (p.accepted_at) {
        desiredFor[p.client_id] = "won";
        continue;
      }
      const expired =
        Date.now() - new Date(p.created_at).getTime() > PROPOSAL_VALID_DAYS * 86400000;
      if (expired && desiredFor[p.client_id] !== "won") desiredFor[p.client_id] = "lost";
    }
    const updates = clients.filter(
      (c) => desiredFor[c.id] && desiredFor[c.id] !== c.status
    );
    if (updates.length) {
      await Promise.all(
        updates.map((c) =>
          supabase.from("clients").update({ status: desiredFor[c.id] }).eq("id", c.id)
        )
      );
  ...
  return (
    <div>
      ...
      <form
        onSubmit={addClient}
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-2 mb-8 p-4 rounded-lg"
      ...
      >
        ...
        <button
          type="submit"
          disabled={saving}
          className="px-5 transition-opacity hover:opacity-90 disabled:opacity-50"
          style={{
            fontFamily: T.mono,
            fontSize: "11px",
      ...
      {loading ? (
        ...
        <p style={{ fontFamily: T.mono, fontSize: "12px", color: T.muted }}>
          No clients yet. Add one above or convert an enquiry.
        </p>
      ) : (
        <div
        ...
        >
          {rows.map((c, i) => (
            <Link
              ...
              className="grid grid-cols-[1fr_100px_auto] md:grid-cols-[1fr_1fr_120px_120px] gap-3 md:gap-4 items-center px-4 sm:px-5 py-4 hover:bg-[#1f1f23] transition-colors"
              style={{
            ...
            >
              ...
              <span
                className="hidden md:inline min-w-0 truncate"
              ...
              <span
                className="hidden md:inline text-right"
                style={{
                  fontFamily: T.mono,
                  fontSize: "10px",
                  color: T.muted,
