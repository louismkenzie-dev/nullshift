...
import { QUOTE_ROWS, formatMoney, opportunityById } from "@/lib/next/fixtures-ops";
...
export default async function OpportunityPage({
  ...
  const x = opportunityById(id);
  if (!x) notFound();

  const d = x.detail;
  const linkedQuotes = d.linkedQuoteIds.map(
    (qid) => QUOTE_ROWS.find((q) => q.id === qid) ?? { id: qid, missing: true as const }
  );
  ...
  return (
    <>
      <p className={s.mono}>
        <Link href="/admin/next/sales?tab=pipeline">Sales &amp; Quotes</Link> / Pipeline /{" "}
        {x.company}
      </p>
      <div className={s.pageHead}>
        <div>
          ...
          <p className={s.lead}>
            {x.clientId ? (
              <Link href={`/admin/next/clients/${x.clientId}`} className={s.rowLink}>
                {x.company}
              </Link>
            ) : (
              x.company
        ...
        <Link href="/admin/next/sales?tab=pipeline&view=board" className={s.btn}>
          Back to board
        </Link>
      </div>
                ...
                {linkedQuotes.map((q) =>
                  "missing" in q ? (
                    <li key={q.id} className={s.listItem}>
                      ...
                      <span className={s.faint}>not in this slice</span>
                    </li>
                  ) : (
                    <li key={q.id} className={s.listItem}>
                      <span>
        ...
        <aside className={o.aside} aria-label="Next action and conversion">
          ...
          <section className={s.card} aria-labelledby="convert">
                ...
                title={
                  x.closed
                    ...
                    : needsReview
                      ...
                      : hasReuse
                        ? "Would reuse the matched record — write not in this slice"
                        : "Would create a new client record — write not in this slice"
                }
              >
            ...
            <p className={s.faint} style={{ margin: "8px 0 0", fontSize: 12 }}>
              Conversion is a server action behind requireStaff() with an audit row; no
              write exists in this prototype.
            </p>
          </section>
        </aside>
      </div>
