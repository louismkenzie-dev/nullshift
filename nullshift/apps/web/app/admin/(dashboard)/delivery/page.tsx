...
export default async function DeliveryPage({
  ...
  return (
    <>
      ...
      <SubTabs base="/admin/delivery" current={tab} tabs={TABS} />
            ...
            {PROJECTS.map((p) => {
              ...
              return (
                <tr key={p.id}>
                  <td>
                    <Link
                      href={`/admin/delivery/projects/${p.id}`}
                      className={s.rowLink}
                    >
                      {p.name}
                    </Link>
                    <div className={s.mono}>
                      <Link href={`/admin/clients/${p.clientId}`}>{p.client}</Link>
                    </div>
                  </td>
                  <td>
    ...
    <tr>
      <td>
        <div className={child ? o.split : undefined}>
          ...
          <div className={s.mono}>
            {w.id} · <Link href={`/admin/clients/${w.clientId}`}>{w.client}</Link> ·
          ...
          {RELEASES.map((r) => (
            <tr key={r.id}>
              <td>
                ...
                <div className={s.mono}>
                  <Link href={`/admin/clients/${r.clientId}`}>{r.client}</Link>
                </div>
      ...
      <section className={`${s.card} ${s.span8}`} aria-labelledby="handover">
        <div className={s.cardTitle}>
          <h2 className={s.h2} id="handover" style={{ margin: 0 }}>
            <Link href={`/admin/clients/${h.clientId}`} className={s.rowLink}>
              {h.client}
            </Link>{" "}
