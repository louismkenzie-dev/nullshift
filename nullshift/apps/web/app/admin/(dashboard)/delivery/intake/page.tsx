...
const INTAKE = "/admin/delivery/intake";
...
export default async function IntakePage({
  ...
  return (
    <>
      <SampleDataNote />
      <p className={s.mono}>
        <Link href="/admin/delivery?tab=queue">Delivery</Link> / Work queue / Intake
      </p>
      ...
      <div className={s.grid12} style={{ marginTop: 24 }}>
        <section className={`${s.card} ${s.span8}`} aria-labelledby="classify">
          ...
          <p className={s.queueMeta} style={{ marginBottom: 12 }}>
            <Link href={`/admin/clients/${fixture.clientId}`} className={s.rowLink}>
              {fixture.client}
            </Link>{" "}
      ...
      <section className={s.card} style={{ marginTop: 24 }} aria-labelledby="mixed">
        ...
        <p className={s.queueMeta}>
          <Link
            href={`/admin/clients/${MIXED_REQUEST.clientId}`}
            className={s.rowLink}
          >
