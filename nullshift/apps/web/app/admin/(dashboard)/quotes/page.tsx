...
import { money, WEBSITE_BUILD_ITEMS } from "@nullshift/ui/format";
...
function QuotesInner() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: p }, { data: c }] = await Promise.all([
      supabase
        .from("proposals")
        .select(
          "id,created_at,client_id,title,summary,line_items,currency,total,status,sent_to"
        )
  ...
  async function createQuote(e: React.FormEvent) {
    ...
    await supabase.from("proposals").insert({
      ...
      title,
      summary,
      line_items: items.filter((i) => i.label.trim()),
  ...
  function openEdit(q: Quote) {
    ...
    setEditItems(
      q.line_items?.length ? q.line_items : [{ label: "", qty: 1, unit_price: 0 }]
    );
  }
    ...
    <div>
      <div className="flex items-end justify-between mb-8">
        ...
        <button
          ...
          className="px-5 h-11 transition-opacity hover:opacity-90"
          style={{
            fontFamily: T.mono,
            fontSize: "0.75rem",
            letterSpacing: "0.06em",
            textTransform: "uppercase",
            background: creating ? "transparent" : T.primary,
            color: creating ? T.fg : T.primaryFg,
            border: creating ? `1px solid ${T.border}` : "none",
            borderRadius: T.r.sm,
          }}
        >
      ...
      {creating && (
        <form
          ...
          className="p-6 rounded-lg mb-8 flex flex-col gap-3"
      ...
      {loading ? (
        ...
        <p style={{ fontFamily: T.mono, fontSize: 12, color: T.muted }}>No quotes yet.</p>
      ) : (
        <div
          ...
          {rows.map((q, i) => (
            ...
            >
              <div className="px-5 py-4 grid grid-cols-[1fr_160px_120px_auto] gap-4 items-center">
                ...
                <span
                  style={{ fontFamily: T.mono, fontSize: "0.95rem", color: T.primary }}
                >
                ...
                <span
                  style={{
                    ...
                    textTransform: "uppercase",
                    color: statusColor[q.status] ?? T.muted,
                  }}
                >
                  ● {q.status}
                </span>
                <div className="flex gap-2 justify-end">
                  ...
                  <button
                    onClick={() => (editId === q.id ? setEditId(null) : openEdit(q))}
                    style={{
                      fontFamily: T.mono,
                      fontSize: 10,
                      textTransform: "uppercase",
                    ...
                    >
                      ...
                      <div className="flex justify-end mt-1">
                        <button
                          onClick={() => saveEdit(q)}
                          disabled={editSaving}
                          className="px-6 h-10 transition-opacity hover:opacity-90 disabled:opacity-50"
                          style={{
                            fontFamily: T.mono,
                            fontSize: "0.75rem",
                            letterSpacing: "0.06em",
    ...
    <div className="flex flex-col gap-2">
      ...
      {items.map((it, i) => {
        ...
        return disc ? (
          <div
            key={i}
            className="grid grid-cols-[1fr_auto_100px_auto] gap-2 items-center p-2 rounded-md"
          ...
          >
            <input
              ...
              onChange={(e) => {
                const a = [...items];
                a[i] = { ...it, label: e.target.value };
                setItems(a);
              }}
            />
            <div className="flex items-center gap-1">
              <span style={{ fontFamily: T.mono, fontSize: "0.82rem", color: T.muted }}>
                −
              </span>
              <input
                type="number"
                ...
                max={100}
                placeholder="%"
                style={{ ...input, width: 72, fontFamily: T.mono }}
                value={Math.abs(Number(it.unit_price)) || ""}
                onChange={(e) => {
                  const a = [...items];
                  a[i] = { ...it, unit_price: -Math.abs(+e.target.value) };
                  setItems(a);
                }}
              />
              <span style={{ fontFamily: T.mono, fontSize: "0.82rem", color: T.muted }}>
            ...
            <span
              ...
              style={{ fontFamily: T.mono, fontSize: "0.8rem", color: T.accent }}
            >
          ...
          <div
            key={i}
            className="grid grid-cols-[1fr_80px_120px_100px_auto] gap-2 items-center"
          >
            <input
            ...
            <input
              ...
              onChange={(e) => {
                const a = [...items];
                a[i] = { ...it, unit_price: +e.target.value };
                setItems(a);
              }}
            />
            <span
      ...
      <div className="flex gap-3">
        ...
        {!totals.hasDiscount && (
          <button
            type="button"
            onClick={() =>
              setItems([...items, { label: "Discount", qty: 1, unit_price: 0 }])
            }
            style={{
              fontFamily: T.mono,
              fontSize: 10,
...
function TotalsDisplay({ totals }: { totals: ReturnType<typeof calcTotals> }) {
  return (
    <>
      ...
      {totals.hasDiscount && (
        <div className="flex items-center justify-between">
          <span style={{ fontFamily: T.mono, fontSize: "0.85rem", color: T.accent }}>
            {totals.discountItem?.label || "Discount"} (−{totals.discountPct}%)
          </span>
          <span style={{ fontFamily: T.mono, fontSize: "0.85rem", color: T.accent }}>
      ...
      <div className="flex items-center justify-between">
        ...
        >
          TOTAL: <span style={{ color: T.primary }}>{money(totals.total)}</span>
        </span>
      </div>
    </>
  );
