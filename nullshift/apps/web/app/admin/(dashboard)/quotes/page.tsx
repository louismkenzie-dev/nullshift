...
/**
 * Quote versions (brief §5.2): 0057 rows through the staff RLS client. Every
 * button is a server action that re-checks staff, preview and the state
  ...
  return (
    <>
      <div className={s.pageHead}>
        <div>
          <p className={s.mono}>
            Sales &amp; Quotes / Quote versions · source: database (migration 0057) ·
            newest {listing.items.length}
          </p>
        ...
        <Link href="/admin/sales?tab=quotes" className={s.btn}>
          Back to Sales
        </Link>
      </div>
          ...
          <table className={s.table}>
            ...
            <tbody>
              {listing.items.map((item) => {
                ...
                const studioHref = `/admin/quotes/${v.id}`;
                const below = isBelowFloor(v.commercial, v.internal);
                return (
                  <tr key={v.id}>
                    <td>
