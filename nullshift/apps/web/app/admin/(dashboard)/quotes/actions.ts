 ...
 * client → audit_log row. Migration 0057 is applied in production, so there
 * is no flag gate: the state machine, second-person approval and optimistic
 * concurrency checks are the only things standing between a click and a
 * write. Ids arriving from the browser are only ever used after the row has
 * been loaded and its relations checked.
 */

const LIST_PATH = "/admin/quotes";
const studioPath = (versionId: string) => `${LIST_PATH}/${versionId}`;
// The estimator policy a new draft is frozen against; the formula version
// names the calculation module revision.
const POLICY_VERSION = CURRENT_POLICY.id;
...
function revalidate(versionId?: string) {
  revalidatePath(LIST_PATH);
  revalidatePath("/admin/sales");
  if (versionId) revalidatePath(studioPath(versionId));
}
...
/**
 * "New quote": creates the opportunity (or attaches to an existing client when
 * a tenant id is supplied) and an empty v1 draft, then lands in the Studio.
 */
export async function createDraftFromForm(formData: FormData): Promise<void> {
  ...
  if (!legalName || !projectLabel)
    toList("err:invalid:client name and project label are required");

  const opp = await createOpportunity({
    legal_name: legalName,
    contact_email: clean(formData.get("contact_email")),
    owner: clean(formData.get("owner")),
    tenant_id: clean(formData.get("tenant_id")),
    stage: "scope_ready",
    source: "admin/quotes",
  });
  ...
  if (!draft.ok) toList(describe(draft, ""));
  redirect(`${studioPath(draft.versionId)}?notice=${encodeURIComponent("ok:draft v1 created")}`);
}
...
// ---------------------------------------------------------------------------
// Studio adapters: the same actions, reporting back to /admin/quotes/[id].
// ---------------------------------------------------------------------------

function toStudio(versionId: string, notice: string): never {
  redirect(`${studioPath(versionId)}?notice=${encodeURIComponent(notice.slice(0, 200))}`);
}
...
const int = (v: FormDataEntryValue | null, fallback = 0): number => {
  const n = Number(String(v ?? "").replace(/[^0-9.-]/g, ""));
  return Number.isFinite(n) ? Math.round(n) : fallback;
};

/** Save the editable Studio fields (brief, scope, commercial build price + route). */
export async function saveDraftFromStudio(formData: FormData): Promise<void> {
  const ref = refFrom(formData);
  const confidence = String(formData.get("confidence") ?? "low");
  const route = String(formData.get("route") ?? "unresolved");
  const r = await saveDraft({
    ...
    content: {
      ...
      scope: {
        ...
        acceptance: lines(formData.get("acceptance")),
      },
      commercial: {
        build_price_minor: int(formData.get("build_price_gbp")) * 100,
        milestones: lines(formData.get("milestones")).map((l) => {
          const m = /^(.*?)\s+(\d{1,3})%?$/.exec(l);
          return m ? { label: m[1], pct: Number(m[2]) } : { label: l, pct: 0 };
        }),
        route: route === "managed" || route === "independent" ? route : "unresolved",
        run_state: String(formData.get("run_state") ?? "Not assessed"),
        grow_options: [],
        transact: String(formData.get("transact") ?? "Unknown"),
      },
    },
  });
  toStudio(ref.version_id, describe(r, "draft saved"));
}

export async function requestApprovalFromStudio(formData: FormData): Promise<void> {
  const ref = refFrom(formData);
  toStudio(ref.version_id, describe(await requestApproval(ref), "sent for internal review"));
}

export async function approveFromStudio(formData: FormData): Promise<void> {
  const ref = refFrom(formData);
  const r = await approveToIssue({ ...ref, reason: clean(formData.get("reason")) });
  toStudio(
    ref.version_id,
    describe(r, r.ok && r.belowFloor ? "approved below floor (reason recorded)" : "approved to issue")
  );
}

export async function returnToDraftFromStudio(formData: FormData): Promise<void> {
  const ref = refFrom(formData);
  toStudio(
    ref.version_id,
    describe(await returnToDraft({ ...ref, reason: clean(formData.get("reason")) }), "returned to draft")
  );
}

export async function issueFromStudio(formData: FormData): Promise<void> {
  const ref = refFrom(formData);
  const r = await issue({ ...ref, expires_at: clean(formData.get("expires_at")) });
  toStudio(
    ref.version_id,
    describe(r, r.ok ? `issued, expires ${r.expiresAt.slice(0, 10)}` : "")
  );
}

export async function nextVersionFromStudio(formData: FormData): Promise<void> {
  const r = await createNextVersion({ version_id: String(formData.get("version_id") ?? "") });
  if (r.ok) toStudio(r.versionId, `ok:draft v${r.versionNo} created`);
  toStudio(String(formData.get("version_id") ?? ""), describe(r, ""));
}
