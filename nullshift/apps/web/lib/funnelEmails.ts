...
import { answeredSummary, resourceName, type Answers, type Recommendation, type Segment } from "@/lib/funnel";
...
export function ownerEmail(opts: {
  ...
  const { name, email, phone, segment, score, answers, recommendation, resourceUrl } = opts;
  const qualified = segment === "qualified";
  ...
  const inner = `
    ...
    <tr><td style="padding:20px 32px 4px">${button(`mailto:${email}?subject=${encodeURIComponent("Re: your project")}`, `Reply to ${name.split(" ")[0] || name} →`)}</td></tr>
  `;
