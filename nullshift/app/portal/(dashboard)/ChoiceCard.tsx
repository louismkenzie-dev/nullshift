...
export function ChoiceCard({ updateId, type, title, body, options, currentChoice, resolved }: Props) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
        <div>
          <span style={{
            ...
            color: colour, background: `${colour}18`, padding: "3px 10px",
            borderRadius: T.r.full, display: "inline-block", marginBottom: 8,
          }}>
            {done ? `${TYPE_LABEL[type]} — answered ✓` : TYPE_LABEL[type]}
          </span>
          <h3 style={{ fontFamily: T.display, fontWeight: 600, fontSize: "1rem", letterSpacing: "-0.01em", color: T.fg, margin: 0 }}>
            {title}
          </h3>
        </div>
      </div>
      ...
      {body && (
        <p style={{ fontFamily: T.sans, fontSize: "0.875rem", lineHeight: 1.6, color: T.muted, marginBottom: 16 }}>
          {body}
        </p>
      )}
            ...
            >
              {opt.image_url && (
                ...
                <img
                  src={opt.image_url}
                  alt={opt.label}
                  style={{ width: "100%", height: 100, objectFit: "cover", borderRadius: T.r.sm, display: "block", marginBottom: 10, border: `1px solid ${T.border}` }}
                />
              )}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <span style={{ fontFamily: T.sans, fontWeight: isChosen ? 600 : 400, fontSize: "0.875rem", color: isChosen ? T.fg : T.muted }}>
                  {opt.label}
                </span>
                {isChosen && (
              ...
              {opt.description && (
                <p style={{ fontFamily: T.sans, fontSize: "0.775rem", lineHeight: 1.5, color: T.faint, marginTop: 4, marginBottom: 0 }}>
                  {opt.description}
                </p>
              )}
            </button>
      ...
      {!done ? (
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button
            ...
            style={{
              height: 42, padding: "0 20px", fontFamily: T.sans, fontSize: "0.875rem", fontWeight: 500,
              letterSpacing: "-0.005em", background: colour, color: T.primaryFg,
              borderRadius: T.r.md, border: "none", cursor: !selected || saving ? "not-allowed" : "pointer",
      ...
      {error && (
        <p style={{ fontFamily: T.sans, fontSize: "0.8rem", color: T.danger, marginTop: 8 }}>{error}</p>
      )}
    </div>
  );
}
