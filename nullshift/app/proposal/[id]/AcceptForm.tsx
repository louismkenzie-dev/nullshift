...
export function AcceptForm({ proposalId }: { proposalId: string }) {
  ...
  if (done) {
    return (
      <p style={{ textAlign: "center", fontFamily: T.mono, fontSize: "0.9rem", color: T.primary }}>
        ✓ Thank you, {name}. Your acceptance has been recorded — we&apos;ll be in touch shortly.
      </p>
    );
  }

  const input: React.CSSProperties = { background: T.bg, border: `1px solid ${T.border}`, borderRadius: 6, padding: "12px 16px", color: T.fg, fontFamily: T.sans, fontSize: "0.95rem", outline: "none", width: "100%" };
  const label: React.CSSProperties = { fontFamily: T.mono, fontSize: "0.72rem", letterSpacing: "0.1em", textTransform: "uppercase", color: T.muted, marginBottom: 8, display: "block" };
    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Full name" style={input} autoComplete="name" />
        </div>
        <div>
      ...
      <div className="mb-6">
        ...
        <div style={{ background: T.bg, border: `1px solid ${T.borderStr}`, borderRadius: 6, height: 96, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontFamily: "var(--font-sans), cursive", fontStyle: "italic", fontSize: "2rem", color: name ? T.fg : T.muted }}>
