...
import { formatCallDate, formatCallTime, money, LONDON_TZ } from "@/lib/format";

type Call = {
  id: string; client_id: string; call_date: string; call_time: string;
  duration_min: number; status: string;
  clients: { name: string; business_name: string | null } | null;
};

type Enquiry = {
  id: string; created_at: string; name: string;
  business_name: string | null; email: string; status: string; source: string;
};

type Client = {
  id: string; name: string; business_name: string | null;
  email: string | null; status: string; brief_completed_at: string | null;
};

type Proposal = {
  id: string; title: string; total: number; currency: string;
  status: string; accepted_at: string | null; client_id: string | null;
};
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [{ data: callsData }, { data: enqData }, { data: clientsData }, { data: proposalsData }] = await Promise.all([
      supabase.from("calls").select("*, clients(name, business_name)").eq("status", "confirmed").gte("call_date", todayStr).order("call_date").order("call_time"),
      supabase.from("enquiries").select("id, created_at, name, business_name, email, status, source").neq("status", "converted").order("created_at", { ascending: false }),
      supabase.from("clients").select("id, name, business_name, email, status, brief_completed_at").order("created_at", { ascending: false }),
      supabase.from("proposals").select("id, title, total, currency, status, accepted_at, client_id").order("created_at", { ascending: false }),
    ]);
  ...
  const missingBriefClients = clients.filter(c => !c.brief_completed_at && !["won", "lost"].includes(c.status));
  ...
  const cells = useMemo(() => {
    ...
    for (let i = 0; i < firstWeekday; i++) out.push({ day: 0, key: `pre-${i}`, inMonth: false });
    for (let d = 1; d <= daysInMonth; d++) {
      out.push({ day: d, key: `${calView.y}-${pad(calView.m + 1)}-${pad(d)}`, inMonth: true });
    }
    return out;
  }, [calView]);
  ...
  if (loading) {
    return <p style={{ fontFamily: T.mono, fontSize: "12px", color: T.muted }}>Loading dashboard…</p>;
  }
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        ...
        <div className="flex flex-col gap-6">
            ...
            {newEnquiries.length === 0 ? (
              ...
              <>
                {newEnquiries.slice(0, 6).map((e, i) => (
                  <Link key={e.id} href="/admin/enquiries"
                    ...
                    <div className="text-right shrink-0 ml-4">
                      <div style={{ fontFamily: T.mono, fontSize: "10px", letterSpacing: "0.06em", textTransform: "uppercase", color: "#facc15" }}>● new</div>
                      <div style={{ fontFamily: T.mono, fontSize: "10px", color: T.muted }}>{new Date(e.created_at).toLocaleDateString("en-GB")}</div>
                    </div>
                  </Link>
                ))}
                {newEnquiries.length > 6 && (
                  <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: "10px", paddingLeft: "12px" }}>
                    <Link href="/admin/enquiries" style={{ fontFamily: T.mono, fontSize: "10px", letterSpacing: "0.08em", textTransform: "uppercase", color: T.primary }}>
          ...
          {/* Missing brief */}
          <Section title="Brief link not sent" label="// ACTION NEEDED" href="/admin/clients">
            {missingBriefClients.length === 0 ? (
              ...
              <>
                {missingBriefClients.slice(0, 6).map((c, i) => (
                  <Link key={c.id} href={`/admin/clients/${c.id}`}
                    ...
                    <div className="shrink-0 ml-4">
                      <span style={{ fontFamily: T.mono, fontSize: "10px", letterSpacing: "0.06em", textTransform: "uppercase", color: T.accent }}>
                        Send brief →
                      </span>
                    </div>
                  </Link>
                ...
                {missingBriefClients.length > 6 && (
                  <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: "10px", paddingLeft: "12px" }}>
                    <Link href="/admin/clients" style={{ fontFamily: T.mono, fontSize: "10px", letterSpacing: "0.08em", textTransform: "uppercase", color: T.primary }}>
            ...
            {nextCall ? (
              <Link href={`/admin/clients/${nextCall.client_id}`} className="block hover:opacity-90 transition-opacity">
                ...
                {nextCall.clients?.business_name && (
                  <div style={{ fontFamily: T.sans, fontSize: "0.82rem", color: T.muted, marginBottom: "10px" }}>
                    {nextCall.clients.name}
                  </div>
                )}
                <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: `${T.primary}14`, border: `1px solid ${T.primary}30` }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: T.primary, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontFamily: T.sans, fontSize: "0.88rem", color: T.fg, fontWeight: 600 }}>{formatCallDate(nextCall.call_date)}</div>
                    <div style={{ fontFamily: T.mono, fontSize: "0.78rem", color: T.primary }}>{formatCallTime(nextCall.call_time)} · {nextCall.duration_min} min</div>
                  </div>
                </div>
                {calls.length > 1 && (
                  <div className="mt-3" style={{ fontFamily: T.mono, fontSize: "10px", color: T.muted, letterSpacing: "0.06em" }}>
              ...
              <div>
                <p style={{ fontFamily: T.sans, fontSize: "0.88rem", color: T.muted, marginBottom: "12px" }}>No upcoming calls booked.</p>
                <Link href="/admin/calendar" style={{ fontFamily: T.mono, fontSize: "10px", letterSpacing: "0.08em", textTransform: "uppercase", color: T.primary }}>
                  View calendar →
                </Link>
              </div>
            )}
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.border}` }}>
            ...
            <div className="p-3">
              <div className="grid grid-cols-7 mb-1">
                {WEEKDAYS.map((w, i) => (
                  <div key={w} className="text-center py-1" style={{ fontFamily: T.mono, fontSize: 8, letterSpacing: "0.08em", textTransform: "uppercase", color: i >= 5 ? `${T.muted}55` : T.muted }}>
                    {w}
                  </div>
                ))}
              </div>
                ...
                {cells.map((c, i) => {
                  ...
                  return (
                    <div key={c.key}
                      ...
                      style={{ background: isToday ? T.primary : hasCall ? `${T.primary}20` : "transparent" }}>
                      <span style={{ fontFamily: T.mono, fontSize: 10, color: isToday ? T.primaryFg : T.fg, fontWeight: isToday ? 700 : 400 }}>
                        {c.day}
                      </span>
                      {hasCall && !isToday && (
                        <span style={{ position: "absolute", bottom: 3, width: 3, height: 3, borderRadius: "50%", background: T.primary }} />
                      )}
                    </div>
                  );
                })}
...
function StatCard({ label, sublabel, value, accent }: { label: string; sublabel: string; value: string; accent: string }) {
  return (
...
function Section({ title, label, href, children }: { title: string; label: string; href: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.border}` }}>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="py-6 px-3 text-center">
      <span style={{ fontFamily: T.sans, fontSize: "0.85rem", color: T.muted }}>{text}</span>
    </div>
  );
}
