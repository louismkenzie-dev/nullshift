...
export function AdminNav({ email }: { email: string }) {
  ...
  return (
    <>
      {/* ── Top bar ────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 flex items-center justify-between px-5 h-14 border-b"
        ...
        <button
          ...
          <span style={{ display: "block", width: 20, height: 1.5, background: T.fg, borderRadius: 2, transition: "transform .2s, opacity .2s", transform: open ? "translateY(6.5px) rotate(45deg)" : "none" }} />
          <span style={{ display: "block", width: 20, height: 1.5, background: T.fg, borderRadius: 2, transition: "opacity .2s", opacity: open ? 0 : 1 }} />
          <span style={{ display: "block", width: 20, height: 1.5, background: T.fg, borderRadius: 2, transition: "transform .2s, opacity .2s", transform: open ? "translateY(-6.5px) rotate(-45deg)" : "none" }} />
        </button>
      </nav>
      ...
      <aside
        ...
        <div className="px-6 py-5 shrink-0 flex flex-col gap-3" style={{ borderTop: `1px solid ${T.border}` }}>
          ...
          <button onClick={signOut} className="w-full h-10 transition-opacity hover:opacity-90"
            style={{ fontFamily: T.mono, fontSize: "11px", letterSpacing: "0.08em", textTransform: "uppercase", color: T.danger, border: `1px solid ${T.danger}40`, borderRadius: "6px" }}>
            Sign out
          </button>
        </div>
      </aside>
